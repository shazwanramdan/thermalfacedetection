...
void LeptonThread::snapImage() {
    ...
    string str0("");
    std::ostringstream sstream0;
    sstream0 << str0 << imageCount;
    std::string imageCountString = sstream0.str();

    string str1("/home/pi/Downloads/LeptonModule-master/software/raspberrypi_video/snap-");
    std::ostringstream sstream1;
    sstream1 << str1 << imageCount;
    std::string str2 = sstream1.str();
    std::string str3;
    str3 = str2 + ".jpg";
    const char * imageCountPath = str3.c_str();

    QImage bigger = myImage.scaled(6 * myImage.size().width(), 6 * myImage.size().height());
    bigger.save(imageCountPath);
    QLabel *popup = new QLabel();
    QPixmap pixmap = QPixmap::fromImage(bigger);
    popup->setPixmap(pixmap);

    std :: string cmd = "lxterminal -e 'python detect_color.py --image snap-"+ imageCountString +".jpg' ";
    system (cmd.c_str ());
